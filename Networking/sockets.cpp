...
void set_up_server_socket(sockaddr_in& dest,int& consocket,int& main_socket,int Portnum)
{
  ...
  //int rcvbuf = 6291456;  
  int rcvbuf = 9291456;    
  fl = setsockopt(main_socket, SOL_SOCKET, SO_RCVBUF, &rcvbuf, sizeof(rcvbuf));
  if (fl<0) { error("set_up_socket:setsockopt set rcvbuf failed"); }
  // int sndbuf = 4194304;     
  int sndbuf = 9191304;     
  fl = setsockopt(main_socket, SOL_SOCKET, SO_SNDBUF, &sndbuf, sizeof(sndbuf));
  if (fl<0) { error("set_up_socket:setsockopt set sndbuf failed"); }
...
void set_up_client_socket(int& mysocket,const char* hostname,int Portnum)
{
  ...
  /*
  * Set the size of the rcvbuf
  */
  int rcvbuf = 9291456;     
  fl = setsockopt(mysocket, SOL_SOCKET, SO_RCVBUF, &rcvbuf, sizeof(rcvbuf));
  if (fl<0) { error("set_up_socket:setsockopt set rcvbuf failed"); }
  int sndbuf = 9191304;      /* a prime number */
  fl = setsockopt(mysocket, SOL_SOCKET, SO_SNDBUF, &sndbuf, sizeof(sndbuf));
  if (fl<0) { error("set_up_socket:setsockopt set sndbuf failed"); }
