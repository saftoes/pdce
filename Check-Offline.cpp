...
void check_mult_triples(const T& key,int N,vector<Data_Files*>& dataF,DataFieldType field_type)
{
    ...
    cout << "[*] gfp field: " << gfp::pr() << endl;
    cout << "[*] MAC key = " << key << endl;
/*
    ifstream* inpf = new ifstream[N];

    for (int i=0; i<N; i++) {
        stringstream filename;
        filename << PREP_DATA_PREFIX << "Triples-p-P" << i;
        cout << "Opening " << filename.str() << endl;
        inpf[i].open(filename.str().c_str(),ios::in | ios::binary);
        if (inpf[i].fail()) { throw file_error(filename.str().c_str()); }
    }

    for (int i = 0; i < N; i++) {
        Sa[i].input(inpf[i], false);
        Sb[i].input(inpf[i], false);
        Sc[i].input(inpf[i], false);

        cout << "[*] Share a = " << Sa[i].get_share() << endl;
        cout << "[*] Share b = " << Sb[i].get_share() << endl;
        cout << "[*] Share c = " << Sc[i].get_share() << endl;
        cout << "[*] MAC a = " << Sa[i].get_mac() << endl;
        cout << "[*] MAC b = " << Sb[i].get_mac() << endl;
        cout << "[*] MAC c = " << Sc[i].get_mac() << endl;
    }
