...
void write_mac_keys(const string& directory, int i, int nplayers, gfp& macp, T& mac2)
{
    ...
    if (mkdir_p(filename.str().c_str()) == -1) {
      cerr << "mkdir_p(" << filename.str() << ") failed\n";
      throw file_error();
    }
    ...
    ifstream test_exist;
    test_exist.open(filename.str().c_str());
    if (!test_exist.good()) {
        cout << "Writing to " << filename.str().c_str() << endl;
        outf.open(filename.str().c_str());
        if (outf.is_open())
            cout << "[*] MAC key open ok ..." << endl;
        else
            cout << "[!] MAC key open failed!" << endl;
