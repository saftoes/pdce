      ...
      for (int j=0; j<N; j++)
        {
            ...
            /*
            cout << "[*] Share a = " << Sa[j].get_share() << endl;
            cout << "[*] Share b = " << Sb[j].get_share() << endl;
            cout << "[*] Share c = " << Sc[j].get_share() << endl;
            cout << "[*] MAC a = " << Sa[j].get_mac() << endl;
            cout << "[*] MAC b = " << Sb[j].get_mac() << endl;
            cout << "[*] MAC c = " << Sc[j].get_mac() << endl;
            */
        }
    }
      ...
      for (int i=0; i<ntrip; i++)
        {
          ...
          make_share(Sa,a,N,key,G);
          for (int j=0; j<N; j++)
            { Sa[j].output(outf[j],true);
              if (j==player)
	        { a.output(outf[j],true);  }
            }
        }
      for (int i=0; i<N; i++)
        { outf[i].close(); }
...
int main(int argc, const char** argv)
{
  ...
  opt.add(
        ...
        1, // Number of args expected.
        0, // Delimiter if expecting multiple args.
        "Default number of tuples to generate for ALL data types (default: 10000)", // Help description.
        "-d", // Flag token.
        "--default" // Flag token.
  );
  opt.add(
  ...
  ofstream outf;
  bigint p;
  generate_online_setup(outf, prep_data_prefix, p, lgp, lg2, sec);
