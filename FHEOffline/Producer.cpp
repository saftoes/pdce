...
/*
 * Our customized four offline protocols
 */
...
RandProducer<T, FD, S>::RandProducer(const FD& FieldD,
    int my_num, int output_thread, bool write_output, string dir) :
    Producer<FD>(output_thread, write_output),
    i(FieldD.num_slots()), value{ FieldD }, mac{ FieldD },
    ri(value), gam_ri(mac)
{
    this->dir = dir;
    value.allocate_slots(FieldD.get_prime());
    mac.allocate_slots(FieldD.get_prime() << limb_size<S>());
    if (write_output)
        this->clear_file(my_num, output_thread, false);
}

template <class T, class FD, class S>
void RandProducer<T, FD, S>::run(const Player& P, const FHE_PK& pk,
    ...
    DistDecrypt<FD>& dd, const T& alphai)
{
    ...
    throw runtime_error("High Gear version of Rand not implemented");
}

template<class T, class FD, class S>
void RandProducer<T, FD, S>::get(Share<T>& r)
{
    if (i >= ri.num_slots()) {
        cout << "[*] Current index: " << i << endl;
        throw overflow_error("ran out of Rand() values");
    }

    r.set_share(ri.element(i));
    r.set_mac(gam_ri.element(i));
    i++;
}

template <class T, class FD, class S>
int RandProducer<T, FD, S>::output(const Player& P, int thread,
    int output_thread)
{
    (void)P;
    (void)thread;
    (void)output_thread;
    throw runtime_error("Output method not implemented");
}

template<class T, class FD, class S>
size_t RandProducer<T, FD, S>::report_size(ReportType type)
{
    return ri.report_size(type) + gam_ri.report_size(type);
}

template <class T, class FD, class S>
int RandProducer<T,FD,S>::sacrifice(const Player& P, MAC_Check<T>& MC)
{
    // NOTE: Fake sacrifice, used to output rand values
    int n_rands = ri.num_slots();
    Rand_Checking(P, MC, n_rands, this->output_thread, *this,
            this->write_output, false, this->dir);
    this->n_slots = n_rands;
    return n_rands;
}
    ...
    ci(values[2]), gam_ai(macs[0]), gam_bi(macs[1]), gam_ci(macs[2])
{
  ...
  // NOTE: These steps are for allocating space of values[] and macs[]
  for (auto& x : values)
    ...
    DistDecrypt<FD>& dd, const T& alphai)
{
    ...
    // NOTE: EC here is of either SummingEncCommit type or SimpleEncCommit_ type
    // Steps a,b,c,d
    timers["Committing"].start();
    // NOTE: Sampling ai / bi with associated ca / cb precomputed
    //       For SimpleEncCommit_ & SummingEncCommit, see SimpleEncCommit.cpp
    //       The generation of (mi, cm) pair relys on ZKPoPK
    EC.next(ai,ca);
    EC.next(bi,cb);
    ...
    // NOTE:
    // Parameters here came from:
    //       ci:      Producer.h line:66
    //       cc:      Producer.cpp line:128
    //       cab:     Producer.cpp line:128
    //       true:    Indicating NewCiphertext
    //       P:       SimpleGenerator.cpp line:65
    //       EC:      SimpleGenerator.cpp line:65
    //       pk:      SimpleGenerator.cpp line:65
    //       dd:      SimpleGenerator.cpp line:65
    // The whole process see [DPSZ12] Protocol Reshare.
    // Relying on dd.run(), i.e., distributed decryption
    Reshare(ci,cc,cab,true,P,EC,pk,dd);
    timers["Resharing"].stop();

    // Step g
    // NOTE:
    // calpha here is the encrypted shared MAC key, i.e.,
    // alpha * a = \sum{gam_ai}
    // alpha * b = \sum{gam_bi}
    // alpha * c = \sum{gam_ci}
    Ciphertext cgam_a(params),cgam_b(params),cgam_c(params);
    timers["Multiplying"].start();
    mul(cgam_a,calpha,ca,pk);     // NOTE: cgam_a = calpha*ca
    mul(cgam_b,calpha,cb,pk);     // NOTE: cgam_b = calpha*cb
    mul(cgam_c,calpha,cc,pk);     // NOTE: cgam_c = calpha*cc
    timers["Multiplying"].stop();

    // Step h
    // NOTE:
    // The whole process see [DPSZ12] Protocol PAngle (calling Protocol Reshare).
    // Relying on dd.run(), i.e., distributed decryption
    // gam_ai / gam_bi / gam_ci are MAC values
...
template class RandProducer<gfp, FFT_Data, bigint>;
template class RandProducer<gf2n_short, P2Data, int>;
