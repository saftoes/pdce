...
// Flag to control the format of output file (readable or not)
#ifdef PSUCADEBUG
...
#define FAKE_NSLOTS 8192
#define RANDOM_FACTOR 3
...
// Rand tuple
template <class T, class FD, class S>
class RandProducer : public SingleSacriFactory< Share<T> >, public Producer<FD>
{
private:
    unsigned int i;

public:
    Plaintext_<FD> value, mac;
    Plaintext<T,FD,S> &ri;
    Plaintext<T,FD,S> &gam_ri;

    RandProducer (const FD& Field, int my_num, int output_thread = 0,
        bool write_output = true, string dir = PREP_DIR);

    string data_type() { return "Rands"; }
    ...
    void get(Share<T>& r);
    void reset() { i = 0; }
    int num_slots() { return ri.num_slots(); }
    size_t report_size(ReportType type);
};

template <class FD>
using RandProducer_ = RandProducer<typename FD::T, FD, typename FD::S>;
