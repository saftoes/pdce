...
void MachineBase::parse_options(int argc, const char** argv)
{
    ...
    opt.add(
          ...
          1, // Number of args expected.
          0, // Delimiter if expecting multiple args.
          "Logarithmic field size (default: 41)", // Help description.
          "-f", // Flag token.
    ...
    opt.add(
          ...
          0, // Number of args expected.
          0, // Delimiter if expecting multiple args.
          "Produce Rand() instead of multiplication triples (default: false)", // Help description.
          "-r", // Flag token.
          "--rand" // Flag token.
    );
    opt.add(
          ...
          0, // Number of args expected.
          0, // Delimiter if expecting multiple args.
          "Produce Rand2() instead of multiplication triples (default: false)", // Help description.
          "-r2", // Flag token.
          "--rand2" // Flag token.
    );
    opt.add(
          ...
          0, // Number of args expected.
          0, // Delimiter if expecting multiple args.
          "Produce RandInRange(l) instead of multiplication triples (default: false)", // Help description.
          "-rl", // Flag token.
          "--randRange" // Flag token.
    );
    opt.add(
          "", // Default.
          0, // Required?
          0, // Number of args expected.
          0, // Delimiter if expecting multiple args.
          "Produce RandExp(l) instead of multiplication triples (default: false)", // Help description.
          "-rx", // Flag token.
          "--randExp" // Flag token.
    );
    opt.add(
          ...
          0, // Number of args expected.
          0, // Delimiter if expecting multiple args.
          "Produce RandBits(l,p) instead of multiplication triples (default: false)", // Help description.
          "-rb", // Flag token.
          "--randBits" // Flag token.
    );
    ...
    if (opt.isSet("--rand"))
        data_type = DATA_RAND;
    else if (opt.isSet("--rand2"))
        data_type = DATA_RAND2;
    else if (opt.isSet("--randRange")) {
        data_type = DATA_RANDRANGE;
        if (this->l <= 0 or this->l >= field_size) {
            throw runtime_error("Invalid parameter for RandInRange(l)");
        }
    }
    else if (opt.isSet("--randExp")) {
        data_type = DATA_RANDEXP;
        if (this->exp <= 0) {
            throw runtime_error("Invalid parameter for RandInRange(l)");
        }
    }
    else if (opt.isSet("--randBits"))
        data_type = DATA_RANDBIT;
    else
        data_type = DATA_TRIPLE;
...
void MachineBase::run()
{
    ...
    cout << "[*] Sent " << 1e-9 * sent << " GB in total" << endl;

    cout << "[*] Produced " << total << " " << item_type() << " in "
            << timer.elapsed() << " seconds" << endl;

    cout << "[*] " << item_type() << " throughput: " << total / timer.elapsed() << tradeoff() << endl;
