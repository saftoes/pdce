        ...
        EC(P, setup.pk, setup.FieldD, timers, machine, thread_num)
{
    if (machine.produce_inputs)
        ...
        case DATA_TRIPLE:
            ...
            break;
        case DATA_RAND:
            producer = new RandProducer_<FD>(setup.FieldD, P.my_num(),
                thread_num, machine.output);
            break;
        default:
...
void SimpleGenerator<T,FD>::run()
{
    ...
    // NOTE: Generate triples & Write to files
    // BUG: nTriplesPerThread here
    while (total < machine.nTriplesPerThread or EC.has_left()) {
    //while (total < machine.nTriplesPerThread) {
        // NOTE: producer is used to generate triples
