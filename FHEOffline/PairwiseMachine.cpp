    ...
    pk(other_pks[N.my_num()]), sk(pk)
{
    ...
    else
    {
        ...
        ofstream outf;
        if (output)
            write_online_setup(outf, setup<FFT_Data>().dirname, p, 40);
    }

    for (int i = 0; i < nthreads; i++)
        if (use_gf2n)
            generators.push_back(new PairwiseGenerator<P2Data>(i, *this, data_type));
        else
            generators.push_back(new PairwiseGenerator<FFT_Data>(i, *this, data_type));
}
...
void PairwiseMachine::setup_keys()
{
    ...
    if (output)
        write_mac_keys(s.dirname, P.my_num(), P.num_players(), setup_p.alphai,
                setup_2.alphai);
